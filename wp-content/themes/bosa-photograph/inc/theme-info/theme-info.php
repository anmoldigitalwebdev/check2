<?php
/**
 * Bosa Photograph - Theme Info Admin Menu
 * @package Bosa Photograph Themes
 * @subpackage Admin
 */
if ( ! class_exists( 'Bosa_Photograph_Theme_Info' ) ) {
    class Bosa_Photograph_Theme_Info{
        ...
        public function prepare_class() {
            ...
            $this->page_title    = $this->theme_name . esc_html__( ' Info', 'bosa-photograph' );
            $this->menu_title    = $this->theme_name . esc_html__( ' Setup', 'bosa-photograph' );
                    ...
                    foreach ( $gs_steps as $gs_step ) {
                        ...
                        if( $is_gs ) {
                            if ( is_plugin_active('advanced-import/advanced-import.php') && is_plugin_active('keon-toolset/keon-toolset.php') && is_plugin_active('kirki/kirki.php') && is_plugin_active('elementor/elementor.php') && is_plugin_active('breadcrumb-navxt/breadcrumb-navxt.php') && is_plugin_active( 'contact-form-7/wp-contact-form-7.php' ) && is_plugin_active( 'bosa-elementor-for-woocommerce/bosa-elementor-for-woocommerce.php' ) ) {
                                echo ('<h3>'.esc_html__('Plugins added successfully. You have completed Step 1.', 'bosa-photograph').'</h3>');
                            echo '</div></div>';
                            }else {
                                if ( ! empty( $title ) ) {
                                    echo '<h3>';
                                ...
                                if ( ! empty( $button_label ) ) {
                                    ...
                                    $button_class = '';
                                    if ( $is_button ) {
                                        $button_class = 'button button-primary button-hero bosa-photograph-install-plugins';
                                    }
        ...
        public function recommended_actions() {
            ...
            if ( ! empty( $recommended_actions ) ) {
                        ...
                        if ( ! empty( $plugin_slug ) ) {
                            ...
                            switch ( $active['needs'] ) {
                                ...
                                case 'install':
                                    ...
                                    $label = esc_html__( 'Install', 'bosa-photograph' );
                                    break;

                                case 'activate':
                                    $class = 'activate-now button button-primary';
                                    $label = esc_html__( 'Activate', 'bosa-photograph' );

                                    break;

                                case 'deactivate':
                                    $class = 'deactivate-now button';
                                    $label = esc_html__( 'Deactivate', 'bosa-photograph' );
                ...
                if ( $hooray ){
                    echo '<span class="hooray">' . esc_html__( 'Hooray! There are no recommended actions for you right now.', 'bosa-photograph' ) . '</span>';
                    echo '<a data-action="reset" id="reset" class="reset-all" href="#">'.esc_html__('Show All Recommended Actions', 'bosa-photograph').'</a>';
                }
                echo '</div>';
            }
        }
                        ...
                        if( !empty( $screenshot) ){
                            ...
                            if ( ! empty( $title ) ) {
                                ...
                                if ( ! empty( $download_link ) ) {
                                    echo '<a href="' . esc_url( $download_link ) . '" class="button button-primary install right">' . esc_html__( 'Download', 'bosa-photograph' ) . '</a>';
                                }
                                if ( ! empty( $preview_link ) ) {
                                    echo '<a class="button button-secondary preview right" target="_blank" href="' . esc_attr( $preview_link ) . '">' . esc_html__( 'Live Preview', 'bosa-photograph' ). '</a>';
                                }
                                echo '</div>';
                                echo '</div>';
                            }
        ...
        */
        ...
        public function style_and_scripts( $hook_suffix ) {
            ...
            wp_enqueue_style( 'bosa-photograph-info-css', get_template_directory_uri() . '/inc/theme-info/assets/css/theme-info.css' );

            if ( 'appearance_page_' . $this->theme_slug . '-info' == $hook_suffix ) {

                wp_enqueue_script( 'bosa-photograph-info-js', get_template_directory_uri() . '/inc/theme-info/assets/js/theme-info.js', array( 'jquery' ) );
                ...
                $count = $this->count_recommended_actions();
                wp_localize_script( 'bosa-photograph-info-js', 'kt_theme_info_object', array(
...
$bosa_photograph_theme_name_config = esc_attr ( wp_get_theme()->get('Name') );
$bosa_photograph_theme_name_config_url = strtolower( str_replace( ' ', '-', $bosa_photograph_theme_name_config ) );
$bosa_photograph_demo_name_config_url = 'photograph';
if( is_child_theme() ){
    $bosa_photograph_demo_name_config_url = str_replace( 'bosa-photograph-', '', $bosa_photograph_theme_name_config_url );
}

$bosa_photograph_config = array(

    // Main welcome title
    /* translators: %s - Theme Name*/
    'info_title' => sprintf( esc_html__( 'Welcome to %s - ', 'bosa-photograph' ), $bosa_photograph_theme_name_config ),

    // Main welcome content
    /* translators: %s - Theme Name*/
    'info_content' => sprintf( esc_html__( '%s is now installed and ready to use. We hope the following information will help and you enjoy using it!', 'bosa-photograph' ), '<b>'.$bosa_photograph_theme_name_config.'</b>' ),
    ...
    'quick_links' => array(
        'theme_url'  => array(
            'text' => __( 'Theme Details', 'bosa-photograph' ),
            'url' => 'https://bosathemes.com/' .$bosa_photograph_theme_name_config_url 
        ),
        'demo_url'  => array(
            'text' => __( 'View Demo', 'bosa-photograph' ),
            'url' => 'https://demo.bosathemes.com/bosa/' .$bosa_photograph_demo_name_config_url
        ),
        'pro_url'  => array(
            'text' => __( 'View Pro Version', 'bosa-photograph' ),
            'url' => 'https://bosathemes.com/bosa-pro'
        ),
        'rate_url'  => array(
            'text' => __( 'Rate This Theme', 'bosa-photograph' ),
            'url' => 'https://wordpress.org/support/theme/' .$bosa_photograph_theme_name_config_url. '/reviews'
        ),  
    ),

    'tabs' => array(
        'getting_started'      => esc_html__( 'Getting Started', 'bosa-photograph' ),
        'recommended_plugins'  => esc_html__( 'Recommended Plugins', 'bosa-photograph' ),
        'support'              => esc_html__( 'Support', 'bosa-photograph' ),
        'changelog'            => esc_html__( 'Changelog', 'bosa-photograph' ),
        'free_pro'             => esc_html__( 'Free VS Pro', 'bosa-photograph' ),
        'demos'                => esc_html__( 'Demos', 'bosa-photograph' ),
        'rating'               => esc_html__( 'Rate Theme', 'bosa-photograph' )
    ),

    /*Getting started tab*/
    'gs_steps' => array(
        'first' => array(
            'title' => esc_html__( 'Step 1 - To fully take advantage of the best our theme can offer, get started.', 'bosa-photograph' ),
            /* translators: %s - Theme Name*/
            'button_label' => sprintf(esc_html__( 'Get started with %s', 'bosa-photograph' ), $bosa_photograph_theme_name_config ),
            'is_button' => true,
        ...
        'second' => array(
            'title' => esc_html__( 'Step 2 - Checkout starter sites (Demos)', 'bosa-photograph' ),
            'desc' => esc_html__( 'All starter demos sites comes attached to Bosa Photograph just ready to pluck one from. Discover them, be enamoured and finally decide on the one you resonate with. If one doesnt catch your fancy dont be afraid to have a go at building a site with your own design.', 'bosa-photograph' ),
            'button_label' => esc_html__( 'View Demo', 'bosa-photograph' ),
            'button_link' => esc_url( 'https://demo.bosathemes.com/bosa/' .$bosa_photograph_demo_name_config_url ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'third' => array (
            'title' => esc_html__( 'Step 3 - Import demo of your choice', 'bosa-photograph' ),
            'desc' => esc_html__( 'Once you have a found a site you like, you can move ahead and import it. It is to be noted that some necessary plugins will be installed to function the site as you see in the demo site. A fresh Wordpress site is recommended to mirror the demo site on your setup. Importing the demo on an already running site will create discrepancies with the site shown on the demo.', 'bosa-photograph' ),
            'button_label' => esc_html__( 'Import Demo', 'bosa-photograph' ),
            'button_link' => esc_url( admin_url( 'themes.php?page=advanced-import' ), 'bosa-photograph' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'fourth' => array(
            'title' => esc_html__( 'Step 4 - Start editing content and making your site!', 'bosa-photograph' ),
            'desc' => esc_html__( 'Once finishing the import you play with the demo to your hearts content.', 'bosa-photograph' ),
        ),
        'fifth' => array (
            'title' => esc_html__( "Step 5 - That's it!", 'bosa-photograph' ),
            'desc' => esc_html__( 'Go live with the website and enjoy it!', 'bosa-photograph' ),
        ),
    ),
    ...
    'support_content' => array(
        'first' => array(
            'title' => esc_html__( 'Need more features?', 'bosa-photograph' ),
            'desc' => esc_html__( 'Upgrade to PRO version for more exciting features and Priority Support.', 'bosa-photograph' ),
            'button_label' => esc_html__( 'Upgrade to PRO', 'bosa-photograph' ),
            'button_link' => esc_url( 'https://bosathemes.com/bosa-pro' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'second' => array (
            'title' => esc_html__( 'Go to Customizer', 'bosa-photograph' ),
            'desc' => esc_html__( 'All Settings, Header & Footer Options, Theme Options, Widgets and Menus are available via Customize screen.', 'bosa-photograph' ),
            'button_label' => esc_html__( 'Go to Customizer', 'bosa-photograph' ),
            'button_link' => esc_url( admin_url( 'customize.php' ) ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'third' => array(
            'title' => esc_html__( 'Documentation', 'bosa-photograph' ),
            'desc' => esc_html__( 'Please check our full documentation for detailed information on how to Setup and Use Bosa Photograph.', 'bosa-photograph' ),
            'button_label' => esc_html__( 'Read full documentation', 'bosa-photograph' ),
            'button_link' => esc_url( 'https://bosathemes.com/docs/bosa' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'fourth' => array (
            'title' => esc_html__( 'Support Forum', 'bosa-photograph' ),
            'desc' => esc_html__( 'Got theme support question or found bug? Simply create a support ticket with your queries and we will get back to you.', 'bosa-photograph' ),
            'button_label' => esc_html__( 'Support Forum', 'bosa-photograph' ),
            'button_link' => esc_url( 'https://bosathemes.com/user-login-register' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'fifth' => array(
            'title' => esc_html__( 'Got sales related question?', 'bosa-photograph' ),
            'desc' => esc_html__( "Have any query before purchase, you are more than welcome to ask.", 'bosa-photograph' ),
            'button_label' => esc_html__( 'Pre-sale Question?', 'bosa-photograph' ),
            'button_link' => esc_url( 'https://bosathemes.com/pre-sale' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'sixth' => array(
            'title' => esc_html__( 'Customization Request', 'bosa-photograph' ),
            'desc' => esc_html__( 'Needed any customization for the theme, you can request from here.', 'bosa-photograph' ),
            'button_label' => esc_html__( 'Customization Request', 'bosa-photograph' ),
    ...
    'free_pro' => array (
        'first'=> array(
            'title' => esc_html__( 'Free VS Pro Features', 'bosa-photograph' ),
            /* translators: %s - Theme Name*/
            'desc' => sprintf( esc_html__( '%s is a multipurpose free WordPress theme. However, Bosa Pro comes with more cool and awesome features that give you a flexible and wide range of options to build a full-fledged website.', 'bosa-photograph' ), $bosa_photograph_theme_name_config ),
            'link_title' => esc_html__( 'Check All Features', 'bosa-photograph' ),
    ...
    'demos' => array (
        'first'=> array(
            'title' => esc_html__( 'Pre-built Demos', 'bosa-photograph' ),
            'desc' => esc_html__ ( 'To get started with ready-made starter site templates.', 'bosa-photograph' ),
            'link_title' => esc_html__( 'View All Demos', 'bosa-photograph' ),
            'link_url' => esc_url( 'https://demo.bosathemes.com/bosa/' .$bosa_photograph_demo_name_config_url ),
            'is_button' => true,
            'recommended_actions' => false,
            'is_new_tab' => true
        ),
    ...
    'rating' => array(
        'first'=> array(
            'title' => esc_html__( 'Show us how much you like our theme.', 'bosa-photograph' ),
            'desc' => esc_html__( 'If you like our work, please give us a moment of your time to rate our theme 5 stars on ', 'bosa-photograph' ) .'<a target="_blank" href="https://wordpress.org/support/theme/' .$bosa_photograph_theme_name_config_url. '/reviews">' . 'wordpress.org' . '</a>' . esc_html__('. It will give us more energy to work on this theme.', 'bosa-photograph' ),
            'link_title' => esc_html__( 'Rate Theme', 'bosa-photograph' ),
            'link_url' => esc_url( 'https://wordpress.org/support/theme/' .$bosa_photograph_theme_name_config_url. '/reviews' ),
            'is_button' => true,
            'is_new_tab' => true
        )
    )
);
return new Bosa_Photograph_Theme_Info( $bosa_photograph_config );